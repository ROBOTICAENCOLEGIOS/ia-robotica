/**
TurboWarp / Scratch 3 Custom Extension - REC PCB1 ARDUINO v1.7.5
Web Serial API @ 115200 baud. Anti-Spam y Timeouts Rápidos. */
(function (Scratch) {
  'use strict';
  ...
  class RecPcb1Arduino {
    ...
    async lightOn(args) {
      ...
      const cmd = `${rgb.r},${rgb.g},${rgb.b}`;
      if (this._lastLedValue[args.LED] === cmd) return;
      this._lastLedValue[args.LED] = cmd;
      await this._sendLine(`AT+LED${args.LED}=${cmd}`);
    }
    ...
    async playNote(args) {
      ...
      const cmd = `${freq},${duration}`;
      if (this._lastNote === cmd) return;
      this._lastNote = cmd;
      await this._sendLine(`AT+NOTE=${cmd}`);
    }

    _normalizeDistanceCm(n) {
      if (typeof n !== 'number' || !Number.isFinite(n) || n === -1 || n === 0 || n < 2 || n > 450) return null;
      return n;
    }
    ...
    distanceCm() {
      ...
      return this._enqueueSerial(async () => {
        ...
        // Timeout ultra rápido (150ms) para evitar atascos
        const linePromise = this._waitForLine(pred, 150)
          .then((ln) => {
    ...
    lineDetected() {
      ...
      const now = Date.now();
      if (now - this._irLastMs < 65) {
        return Promise.resolve(this._irLastVal);
      }
      return this._enqueueSerial(async () => {
        const pred = (ln) => ln === '0' || ln === '1';
        // Timeout ultra rápido (100ms) para evitar atascos
        const linePromise = this._waitForLine(pred, 100).then((ln) => ln === '1').catch(() => false);
        await this._sendLineRaw('AT+IR');
        const out = await linePromise;
        this._irLastMs = Date.now();
        this._irLastVal = out;
        return out;
      });
    }
