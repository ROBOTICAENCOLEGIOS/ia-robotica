...
(function (Scratch) {
  ...
  class RecPcb1Arduino {
    ...
    getInfo() {
      return {
        ...
        menus: {
          ...
          musicalNotes: { items: [{ text: 'DO (C4)', value: '262' }, { text: 'RE (D4)', value: '294' }, { text: 'MI (E4)', value: '330' }, { text: 'FA (F4)', value: '349' }, { text: 'SOL (G4)', value: '392' }, { text: 'LA (A4)', value: '440' }, { text: 'SI (B4)', value: '494' }, { text: 'DO (C5)', value: '523' }] },
        },
      };
    }
    ...
    _waitForLine(predicate, timeoutMs) { return new Promise((resolve, reject) => { const w = { predicate, resolve, reject }; this._lineWaiters.push(w); if (timeoutMs > 0) { w.timer = setTimeout(() => { const j = this._lineWaiters.indexOf(w); if (j >= 0) this._lineWaiters.splice(j, 1); reject(new Error('Timeout')); }, timeoutMs); } const origResolve = w.resolve; w.resolve = (val) => { if (w.timer) clearTimeout(w.timer); origResolve(val); }; }); }

    _startReadLoop() {
      if (this._readLoopRunning || !this._activePort || !this._activePort.readable) return;
      this._readLoopRunning = true;
      const run = async () => { try { while (this._activePort && this._readLoopRunning) { const reader = this._activePort.readable.getReader(); try { for (;;) { const { value, done } = await reader.read(); if (done) break; if (value && value.byteLength) this._feedBytes(value); } } finally { reader.releaseLock(); } } } catch (_) {} finally { this._readLoopRunning = false; } };
      run();
    }

    _feedBytes(u8) { this._rxRemainder += this.decoder.decode(u8, { stream: true }); let idx; while ((idx = this._rxRemainder.indexOf('\n')) >= 0) { const line = this._rxRemainder.slice(0, idx).replace(/\r$/, '').trim(); this._rxRemainder = this._rxRemainder.slice(idx + 1); if (line) this._dispatchLine(line); } }
    _dispatchLine(line) { for (let i = 0; i < this._lineWaiters.length; i++) { const w = this._lineWaiters[i]; if (w.predicate(line)) { this._lineWaiters.splice(i, 1); w.resolve(line); return; } } }
    _hexToRgb(hex) { let s = String(hex).trim(); if (s.startsWith('#')) s = s.slice(1); if (s.length === 3) s = s.split('').map((ch) => ch + ch).join(''); if (!/^[0-9a-fA-F]{6}$/.test(s)) return { r: 0, g: 0, b: 0 }; return { r: parseInt(s.slice(0, 2), 16), g: parseInt(s.slice(2, 4), 16), b: parseInt(s.slice(4, 6), 16) }; }

    async _setMotor(side, value) { if ((side === 'IZQ' || side === 'DER') && this._lastMotorValue[side] === value) return; if (side === 'IZQ' || side === 'DER') this._lastMotorValue[side] = value; await this._sendLine(`AT+M_${side}=${value}`); }
    async moveForward(args) { const speed = Math.round((Math.abs(args.PCT) / 100) * 255); await this._setMotor(args.SIDE, speed); }
    async moveBackward(args) { const speed = Math.round((Math.abs(args.PCT) / 100) * 255) * -1; await this._setMotor(args.SIDE, speed); }
    async stopMotor(args) { if (args.WHICH === 'AMBOS') { if (this._lastMotorValue['IZQ'] === 0 && this._lastMotorValue['DER'] === 0) return; this._lastMotorValue['IZQ'] = 0; this._lastMotorValue['DER'] = 0; await this._sendLine('AT+MOTOR=STOP'); return; } await this._setMotor(args.WHICH, 0); }
    ...
    _normalizeDistanceCm(n) { if (typeof n !== 'number' || !Number.isFinite(n) || n === -1 || n === 0 || n < 2 || n > 450) return null; return n; }
    _filterDistanceCm(n) { if (n == null) return null; if (this._distanceEma == null) { this._distanceEma = n; return n; } const alpha = 0.35; this._distanceEma = Math.round(this._distanceEma * (1 - alpha) + n * alpha); return this._distanceEma; }
    _distanceDisplayCm(v) { if (typeof v !== 'number' || !Number.isFinite(v) || v <= 0) return 999; return Math.round(v); }
    ...
    distanceCm() {
      ...
      return this._enqueueSerial(async () => {
        ...
        const linePromise = this._waitForLine(pred, 5000).then((ln) => { const raw = parseInt(String(ln).trim(), 10); const norm = this._normalizeDistanceCm(raw); if (norm != null) { this._distanceLastGood = norm; return this._filterDistanceCm(norm); } throw new Error("Invalid"); }).catch(() => (this._distanceLastGood != null ? this._distanceLastGood : 999));
    ...
    lineDetected() { if (!this._connected()) return false; return this._enqueueSerial(async () => { const pred = (ln) => ln === '0' || ln === '1'; const linePromise = this._waitForLine(pred, 5000).then((ln) => ln === '1').catch(() => false); await this._sendLineRaw('AT+IR'); return await linePromise; }); }
  }
