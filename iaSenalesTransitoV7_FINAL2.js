class IASenalesTransitoV7 {
  ...
  // MÉTODO CORREGIDO: getUserMedia() debe ser inmediato al gesto del usuario
  start() {
    if (!this.model) {
    ...
    // Llamar a getUserMedia() inmediatamente sin await antes
  ...
  // Separar la lógica de manejo de cámara para no interferir con el gesto del usuario
  async _handleCameraStream(stream) {
    try {
      this.stream = stream;
