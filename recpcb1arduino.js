...
class RecPcb1Arduino { 
  ...
  // OVERLAY BUTTON: Hacer visible el iframe oculto de Scratch
  _createOverlayButton() {
    // Hacer visible el iframe oculto de Scratch
    if (window.frameElement) {
      this._originalIframeCss = window.frameElement.style.cssText;
      window.frameElement.style.cssText = 'position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; z-index: 999999; border: none; display: block; background: transparent;';
    }
    
    // Forzar estilos del documento para que el botón tenga dónde dibujarse
    ...
    this._overlayButton.style.cssText = 'position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0, 0, 0, 0.8); display: flex; align-items: center; justify-content: center; z-index: 999999; cursor: pointer;';
    this._overlayButton.innerHTML = '<div style="padding: 30px; background: #ff4444; color: white; font-size: 30px; border-radius: 15px; font-weight: bold; border: 4px solid #00ff00;">HAGA CLIC AQUÍ PARA CONECTAR ARDUINO</div>';
    document.body.appendChild(this._overlayButton);

    console.log('BOTÓN CREADO Y VISIBLE');
    this._overlayButton.addEventListener('click', () => { this._directConnect(); });
  }

  // REMOVE OVERLAY: Volver a ocultar el iframe
  _removeOverlayButton() {
    if (this._overlayButton) {
      this._overlayButton.remove();
      this._overlayButton = null;
    }
    if (window.frameElement && this._originalIframeCss !== undefined) {
      window.frameElement.style.cssText = this._originalIframeCss;
    }
  }
    ...
    try {
      ...
      navigator.serial.requestPort()
        ...
        .catch(e => {
          ...
          if (this._overlayButton) {
            this._overlayButton.innerHTML = '<div style="padding: 30px; background: #ff0000; color: white; font-size: 30px; border-radius: 15px; font-weight: bold; border: 4px solid #ff0000;">ERROR AL CONECTAR<br><span style="font-size: 20px;">HAGA CLIC PARA REINTENTAR</span></div>';
          }
        });
    } catch (e) {
      console.error('Error en requestPort:', e);
      if (this._overlayButton) {
        this._overlayButton.innerHTML = '<div style="padding: 30px; background: #ff0000; color: white; font-size: 30px; border-radius: 15px; font-weight: bold; border: 4px solid #ff0000;">ERROR CRÍTICO<br><span style="font-size: 20px;">RECARGUE LA PÁGINA</span></div>';
      }
    }
  }
