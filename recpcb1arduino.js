...
class RecPcb1Arduino { 
  ...
  // CÓDIGO EXACTO DE VISIBILIDAD EXTREMA
  _createOverlayButton() {
    console.log('CREANDO BOTÓN EXTREMO');
    
    if (window.frameElement) {
        this._oldStyle = window.frameElement.style.cssText;
        // El z-index debe ser el máximo posible: 2147483647
        window.frameElement.setAttribute('style', 'position:fixed !important; top:0 !important; left:0 !important; width:100vw !important; height:100vh !important; z-index:2147483647 !important; display:block !important; background:rgba(0,0,0,0.7) !important; border:none !important;');
        console.log('IFRAME FORZADO CON setAttribute');
    }

    this._overlayButton = document.createElement('div');
    this._overlayButton.setAttribute('style', 'position:fixed; top:0; left:0; width:100vw; height:100vh; display:flex; align-items:center; justify-content:center; cursor:pointer; z-index:2147483647; background:rgba(255,0,0,0.4);');
    
    this._overlayButton.innerHTML = `
        <div style="padding:50px; background:#ff0000; color:white; border:10px solid white; border-radius:30px; text-align:center; box-shadow:0 0 100px black;">
            <h1 style="font-size:50px; margin:0; font-family:sans-serif;">⚠️ CONECTAR ROBOT</h1>
            <p style="font-size:25px;">HAZ CLIC AQUÍ PARA ABRIR EL PUERTO SERIAL</p>
        </div>
    `;
    ...
    console.log('BOTÓN AÑADIDO AL DOCUMENTO');
  }

  // LIMPIEZA EXTREMA: Restaurar con setAttribute
  _removeOverlayButton() {
    console.log('ELIMINANDO BOTÓN EXTREMO');
    ...
    if (window.frameElement && this._oldStyle !== undefined) {
        window.frameElement.setAttribute('style', this._oldStyle);
        console.log('IFRAME RESTAURADO');
    }
  }
    ...
    try {
      navigator.serial.requestPort()
        ...
        .catch(e => {
          ...
          // Mostrar error recreando el botón
          this._createOverlayButton();
          if (this._overlayButton) {
            this._overlayButton.innerHTML = `
                <div style="padding:50px; background:#ff0000; color:white; border:10px solid white; border-radius:30px; text-align:center; box-shadow:0 0 100px black;">
                    <h1 style="font-size:50px; margin:0; font-family:sans-serif;">❌ ERROR AL CONECTAR</h1>
                    <p style="font-size:25px;">HAZ CLIC PARA REINTENTAR</p>
                </div>
            `;
          }
        });
      ...
      if (this._overlayButton) {
        this._overlayButton.innerHTML = `
            <div style="padding:50px; background:#ff0000; color:white; border:10px solid white; border-radius:30px; text-align:center; box-shadow:0 0 100px black;">
                <h1 style="font-size:50px; margin:0; font-family:sans-serif;">❌ ERROR CRÍTICO</h1>
                <p style="font-size:25px;">RECARGUE LA PÁGINA</p>
            </div>
        `;
      }
    }
