...
(function (Scratch) {
  ...
  class IAVisionRECPro {
    ...
    _loadScript(url) { return new Promise((resolve) => { const script = document.createElement('script'); script.src = url; script.onload = resolve; document.head.appendChild(script); }); }
    ...
    getInfo() {
      return {
        ...
        blocks: [
          ...
          "---",
          { opcode: 'setVideoPos', blockType: Scratch.BlockType.COMMAND, text: 'Mover cámara a x: [X] y: [Y]', arguments: { X: { type: Scratch.ArgumentType.NUMBER, defaultValue: 10 }, Y: { type: Scratch.ArgumentType.NUMBER, defaultValue: 10 } } },
          { opcode: 'setVideoSize', blockType: Scratch.BlockType.COMMAND, text: 'Tamaño de cámara al [SIZE] %', arguments: { SIZE: { type: Scratch.ArgumentType.NUMBER, defaultValue: 40 } } },
          "---",
    ...
    async iniciarIA() {
      ...
      try {
        ...
        Object.assign(this.video.style, { position: 'fixed', zIndex: '1000', border: '3px solid #FF5733', borderRadius: '10px', left: '10px', top: '10px', width: '240px', pointerEvents: 'none', transform: 'scaleX(-1)' });
        ...
        faceMesh.onResults((results) => { this.facesDetected = results.multiFaceLandmarks ? results.multiFaceLandmarks.length : 0; });

        const camera = new window.Camera(this.video, { onFrame: async () => { if (this.video && this.video.readyState >= 2) { await hands.send({image: this.video}); await faceMesh.send({image: this.video}); } }, width: 480, height: 360 });
        camera.start();
        this.status = "Listo para detectar";
      } catch (err) { this.status = "Error: Sin cámara"; }
    }

    detenerIA() { if (this.video) { this.video.srcObject.getTracks().forEach(t => t.stop()); this.video.remove(); this.video = null; } this.status = "Apagado"; this.facesDetected = 0; this.handsDetected = 0; this.isPinching = false; }
