/**
 * IA: Visión REC Pro - VERSIÓN ESTABLE
 * Solución al error 'window.Hands is not a constructor' y limpieza de bloques.
 */
...
(function (Scratch) {
  ...
  class IAVisionRECPro {
    ...
    // Función para asegurar que las librerías externas existan antes de usarlas
    async _ensureLibraries() {
      if (window.Hands && window.FaceMesh && window.Camera) return true;
      
      this.status = "Cargando librerías...";
      const scripts = [
    ...
    async iniciarIA() {
      ...
      try {
        await this._ensureLibraries(); // Forzar carga antes de instanciar
        
        this.status = "Iniciando cámara...";
